			...
			else
			{
				...
				if (plugin.manager.isValidArena(arenaName))
				{
					...
					try
					{
						...
						{
							CommandUtil.sendMessageToPlayer(player, ChatColor.RED + "" + ChatColor.STRIKETHROUGH + "----------" + ChatColor.GOLD + "Spawn Points" + ChatColor.RED + "" + ChatColor.STRIKETHROUGH + "----------");
							CommandUtil.sendMessageToPlayer(player, ChatColor.GREEN + "Total doors: " + game.getInGameManager().getDoors().size());
							for (int i = 0; i < game.getInGameManager().getDoors().size(); i++)
							{
								Door door = game.getInGameManager().getDoors().get(i);
